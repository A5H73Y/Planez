 ...
 * If Planez does not link to a Economy plugin, all attempted purchases will be successful.
 */
public class EconomyController {
    ...
    private boolean canRegisterEconomy() {
        Plugin vault = planez.getServer().getPluginManager().getPlugin("Vault");

        if (vault == null || !vault.isEnabled())
            return false;

        RegisteredServiceProvider<Economy> economyProvider =
                planez.getServer().getServicesManager().getRegistration(net.milkbowl.vault.economy.Economy.class);
